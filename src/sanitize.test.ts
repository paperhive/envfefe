...
describe('sanitize', () => {
  describe('boolean()', () => {
    it('should sanitize true', () => {
      expect(sanitize.boolean('true')).to.equal(true);
    });

    it('should sanitize false', () => {
      expect(sanitize.boolean('false')).to.equal(false);
    });
  ...
  describe('json()', () => {
    it('should sanitize a JSON string', () => {
      expect(sanitize.json('{"foo": 1}')).to.eql({foo: 1});
    });
  ...
  describe('number()', () => {
    it('should sanitize a number', () => {
      expect(sanitize.number('1.337')).to.eql(1.337);
    });

    it('should sanitize an integer', () => {
      expect(sanitize.number('-2')).to.eql(-2);
    });
  ...
  describe('string()', () => {
    it('should return the input', () => {
      expect(sanitize.string('foo')).to.equal('foo');
    });
  });
});
