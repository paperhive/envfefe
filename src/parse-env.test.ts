...
describe("parse", () => {
  ...
  describe("parseEnvValue()", () => {
    it("should return a parsed variable", () => {
      ...
      expect(
        parseEnvValue({ sanitize: parseNumber(), name: "NUMBER_VAR" })
      ).to.equal(1.337);
    });
  ...
  describe("parseEnv()", () => {
    it("should return a parsed object with shorthand notation", () => {
      ...
      expect(parseEnv({ booleanVar: parseBoolean() })).to.eql({
        booleanVar: true,
      });
    });
    ...
    it("should return a parsed object with explicit notation", () => {
      process.env.BOOLEAN_VAR = "true";
      expect(
        parseEnv({ booleanVar: { sanitize: parseBoolean(), optional: true } })
    ...
    it("should return a parsed object with provided name", () => {
      ...
      expect(
        parseEnv({
          booleanVar: { sanitize: parseBoolean(), name: "WEIRD_VAR" },
        })
      ).to.eql({ booleanVar: true });
    });
    ...
    it("should return a parsed object with undefined value if unset", () => {
      expect(
        parseEnv({ optionalVar: { sanitize: parseBoolean(), optional: true } })
      ).to.eql({ optionalVar: undefined });
    });

    it("should return a parsed object with default value if provided", () => {
      expect(
        parseEnv({ optionalVar: { sanitize: parseBoolean(), default: false } })
